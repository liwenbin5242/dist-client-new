...
export default {
  ...
  previewUrl: function(username, file, token) {
    if (token) {
      return `${this.baseUrl}/file/${username}${file.path}${encodeURIComponent(file.name)}?disk-token=${token}`
    } else {
  ...
  download: function(username, file, token) {
    fileApi.isAllowDownload().then(() => {
      encodeURI()
      let url = `${this.baseUrl}/file/${username}${file.path}${encodeURIComponent(file.name)}?disk-token=${token}&o=download`
  ...
  packageDownload: function(fileIds, token) {
    fileApi.isAllowDownload().then(() => {
      window.open(`${this.baseUrl}/packageDownload?fileIds=${fileIds}&disk-token=${token}`, '_self')
    })
  },
  // 共享文件下载
